...
model.physical_constant.f = ...
    model.physical_constant.f( ...
    2:(model.grid.MX(1)+1),2:(model.grid.MX(2)+1),:,:);
...
%     %     data = nan(1201,1401,1,N_t);
    ...
    data=ncread( [ ...
        model.folder.data 'surface_summer.nc'],name_data );
    data = data(2:(model.grid.MX(1)+1),2:(model.grid.MX(2)+1),:,:);
    N_t = size(data,3);
%     t_final = t_ini-1 + N_t;

...
a = 1/2 * fct_a(XP,model); % nx d d
a=reshape(a,[MX 2 2]);% n m d d
clear XP

%%

% xref= dX(1)*(0:MX(1)-1)+BOX(1,1);
% yref= dX(2)*(0:MX(2)-1)+BOX(1,2);
% image(xref,yref,1e-5*(a(:,end:-1:1,1,1)+a(:,end:-1:1,2,2)));
% plot_dot_obs(model)
% keyboard

function tau = matrix_tau_homogene_periodic_old(model)
...
for I=1:M
    ...
    if i>1
        if i>2
            ...
            tau(I,end,j)= a(ia-1,ja,1,1)/(dx^2);
        end
        if j>1
            tau(I,i-1,j-1)= (a(ia-1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        else
            tau(I,i-1,end)= (a(ia-1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);            
        end
        if j<My
            tau(I,i-1,j+1)= - (a(ia-1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        else
            tau(I,i-1,1)= - (a(ia-1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        end
    else
        tau(I,end-1,j)= a(ia-1,ja,1,1)/(dx^2);
        if j>1
            tau(I,end,j-1)= (a(ia-1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        else
            tau(I,end,end)= (a(ia-1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        end
        if j<My
            tau(I,end,j+1)= - (a(ia-1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        else
            tau(I,end,1)= - (a(ia-1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        end
    end
    if j>2
        tau(I,i,j-2)=a(ia,ja-1,2,2)/(dy^2);
    else
        tau(I,i,end+j-2)=a(ia,ja-1,2,2)/(dy^2);
    end
    ...
    if j<My-1
        ...
        tau(I,i,j-My+2)= a(ia,ja+1,2,2)/(dy^2);
    end
    if i<Mx
        if j>1
            tau(I,i+1,j-1)= - (a(ia+1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        ...
        if i<Mx-1
            ...
            tau(I,1,j)= a(ia+1,ja,1,1)/(dx^2);
        end
    else
        if j>1
            tau(I,1,j-1)= - (a(ia+1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        else
            tau(I,1,end)= - (a(ia+1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        end
        if j<My
            tau(I,1,j+1)= (a(ia+1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        else
            tau(I,1,1)= (a(ia+1,ja,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        end
        tau(I,2,j)= a(ia+1,ja,1,1)/(dx^2);
    end
end

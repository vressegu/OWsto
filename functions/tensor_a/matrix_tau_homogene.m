function tau = matrix_tau_homogene(model)
...
for I=1:M
    ...
    if i>1
        if i>2
            tau(i,j,i-2,j)= a(ia-1,ja,1,1)/(dx^2);
        end
        if j>1
            tau(i,j,i-1,j-1)= (a(ia-1,ja,1,2)+a(ia,ja-1,1,2))/(dx*dy);
        end
        if j<My-1
            tau(i,j,i-1,j+1)= - (a(ia-1,j,1,2)+a(ia,ja+1,1,2))/(dx*dy);
        end
    end
    if j>2
        tau(i,j,i,j-2)=a(ia,ja-1,2,2)/(dy^2);
    end
    tau(i,j,i,j)= - (a(ia+1,ja,1,1)+a(ia-1,ja,1,1))/(dx^2) ...

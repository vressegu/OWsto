...
type_data = [ type_data '_smooth_' num2str(smoothing) ...
    '_Mx_LR_' num2str(Mx_LR) ];
...
if model.filtering.smoothing
    ...
    if  Mx_LR < Mx_HR
        ...
        for k_t=1:size(w,4)
            %     figure;imagesc(w(:,:,1));
            w(:,:,1,k_t) = conv2(w(:,:,1,k_t),filter_perso,'same');
            w(:,:,2,k_t) = conv2(w(:,:,2,k_t),filter_perso,'same');
        end
...
save([model.folder.folder_simu '/multi_time_save_file.mat']);

%% Classical mixing criterion
...
[alpha2,states] = fct_mix_sto(model,r.^2,sigma_w);
